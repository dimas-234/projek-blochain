...
function showPage(page) {
	...
	if (!target) {
		console.warn('showPage: target not found:', page);
		return;
	}
	...
	// close collapsed navbar on small screens (Bootstrap)
...
// make the inline anchors using showPage() not follow the href="#"
document.addEventListener('click', function (e) {
	const el = e.target.closest('a[onclick^="showPage("]');
	if (!el) return;
	e.preventDefault();
...
document.getElementById('btn-consensus').onclick = async () => {
	try {
		...
		for (let i = 0; i < maxLen; i++) {
			// Kumpulkan versi blok ke-i dari semua node
			const candidates = users
				.map((u) => chainsConsensus[u][i])
				.filter((b) => b && !b.invalid);

			if (candidates.length === 0) continue; // Semua rusak, lewati

			// Pilih blok mayoritas (berdasar data & hash)
			const freq = {};
			...
			// Terapkan blok mayoritas ke semua node
			for (const u of users) {
				const chain = chainsConsensus[u];
				if (!chain[i]) continue;

				// Jika blok invalid, timpa dengan mayoritas
				if (chain[i].invalid) {
					chain[i] = { ...majority, invalid: false };
				}

				// Perbaiki previousHash untuk blok berikutnya
				if (i > 0 && chain[i]) {
